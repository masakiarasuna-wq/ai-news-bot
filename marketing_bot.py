...
DISCORD_BLUE = 0xEB459E  # マーケティング用：ピンク
...
MAX_PER_SOURCE = 10  # 1ヶ月分なので多めに取得
...
def summarize_with_groq(articles: list[dict]) -> dict:
    ...
    prompt = f"""以下は過去1ヶ月のマーケティング関連ニュース記事の一覧です。
...
{{
  "intro": "今週のマーケティングトレンドを3文で説明（AIの話題は除く）",
  "articles": [
    {{
...
def main() -> None:
    ...
    send_embed({
        ...
        "embeds": [{
            "title": f"📣 {today_str}のマーケティングニュース",
